...
import { ConsultationService, ChatMessage } from '../../services/consultation.service';
        ...
        <div class="max-w-7xl mx-auto px-4 py-4">
          <div class="flex items-center justify-between">
            <div class="flex items-center">
              ...
              <div>
                <h1 class="text-2xl font-bold text-gray-900">Consultation #{{ consultationId() }}</h1>
                @if (consultation()) {
                  <p class="text-sm text-gray-600">Created {{ formatDate(consultation().created_at) }}</p>
                }
              </div>
            </div>
            @if (consultation()) {
        ...
        @if (errorMessage() && !loading()) {
          <div class="bg-red-50 border border-red-200 rounded-lg p-4">
            <div class="flex">
              ...
              <button (click)="loadConsultation()" class="text-red-600 hover:text-red-800 text-sm font-medium">
                Retry
              </button>
            </div>
          </div>
        ...
        @if (consultation() && !loading()) {
          <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div class="bg-white rounded-lg shadow-md overflow-hidden">
                ...
                <div class="h-96 overflow-y-auto p-4 space-y-4" #chatContainer>
                  @for (message of chatMessages(); track message.id) {
                    <div [class]="message.sender === 'patient' ? 'flex justify-end' : 'flex justify-start'">
                      <div [class]="message.sender === 'patient' ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-900'" 
            ...
            <div class="space-y-6">
              ...
              <div class="bg-white rounded-lg shadow-md p-6">
                ...
                <div class="space-y-3">
                  <div>
                    ...
                    <span [class]="getStatusClass(consultation().status)" class="ml-2">
                      {{ consultation().status }}
                    </span>
                  </div>
                  <div>
                    <span class="text-sm font-medium text-gray-700">Language:</span>
                    <span class="text-sm text-gray-600 ml-2">{{ consultation().language?.toUpperCase() }}</span>
                  </div>
                  <div>
                    <span class="text-sm font-medium text-gray-700">Created:</span>
                    <span class="text-sm text-gray-600 ml-2">{{ formatDateTime(consultation().created_at) }}</span>
                  </div>
                  @if (consultation().updated_at !== consultation().created_at) {
                    <div>
                      <span class="text-sm font-medium text-gray-700">Updated:</span>
                      <span class="text-sm text-gray-600 ml-2">{{ formatDateTime(consultation().updated_at) }}</span>
                    </div>
                  }
                </div>
                ...
                @if (consultation().transcript) {
                  <div class="mt-4 pt-4 border-t">
                    <span class="text-sm font-medium text-gray-700 block mb-2">Original Description:</span>
                    <p class="text-sm text-gray-600 bg-gray-50 p-3 rounded">{{ consultation().transcript }}</p>
                  </div>
                }

                @if (consultation().audio_url) {
                  <div class="mt-4 pt-4 border-t">
                    <span class="text-sm font-medium text-gray-700 block mb-2">Audio Recording:</span>
                    <audio controls class="w-full">
                      <source [src]="consultation().audio_url" type="audio/webm">
              ...
              <!-- Uploaded Files -->
              @if (consultation().file_attachments && consultation().file_attachments.length > 0) {
                <div class="bg-white rounded-lg shadow-md p-6">
                  <h3 class="text-lg font-semibold text-gray-900 mb-4">Files ({{ consultation().file_attachments.length }})</h3>
                  <div class="space-y-3">
                    @for (file of consultation().file_attachments; track file.id) {
...
export class ConsultationDetailPage implements OnInit, OnDestroy {
  ...
  loadConsultation() {
    ...
    this.consultationService.getById(id).subscribe({
      ...
      error: (error: any) => {
        this.errorMessage.set(error?.error?.detail || error?.message || 'Failed to load consultation');
  ...
  sendMessage() {
    ...
    this.consultationService.addChatMessage(id, {
      sender: 'patient',
      ...
      error: (error: any) => {
        this.errorMessage.set(error?.error?.detail || error?.message || 'Failed to send message');
  ...
  onFileSelected(event: any) {
    ...
    this.consultationService.uploadFile(id, file).subscribe({
      ...
      error: (error: any) => {
        this.errorMessage.set(error?.error?.detail || error?.message || 'Failed to upload file');
  ...
  getStatusClass(status: string): string {
    ...
    switch (status?.toLowerCase()) {
      ...
      case 'active':
      case 'in_progress': return baseClass + 'status-active';
